...
[придумать какой-то мемный слайдик про "интровертный" подход]
...
<br><br>
О каком доверии к тестам в таком случае может идти речь, если мы знаем, что тесты, которые должны обеспечивать качество нашего продукта — зависят от чего-то такого-же случайного, как цена, которую указывают продавцы товаров на маркетплейсках до скидки [если статью будет от озона, то надо бы персмотреть этот подкол :pekaface:]
<br><br>
2. **Скорость** 
<br><br>
...
Вырисовывается какая-то грустная картина, тесты будут долгие, нестабильные, часть из них будет вообще нереализуема или запускать их можно будет только вручную и то очень осторожно, а в «идеале» все вместе. И это все при условии, что какой-то тестовый стенд вообще будет предоставлен вендором.
...
Не лучшая практика, но имеет право на жизнь, а вместе с ним и серьезные нюансы:
    ...
    3. Как-будто можно добавить что-то еще
[ТУТ СЛАЙД, КАК ВСТРАИВАЕТСЯ ЭТОТ МОК В БИЗНЕС ПРОЦЕСС]
...
<br><br>
    ...
    <br><br>
    5. Это пункт вытекает из самого определения, но все же готовый инструмент ничего не знает о вашей специфике, что 
    <br><br>
[ТУТ СЛАЙД, КАК ВСТРАИВАЕТСЯ ЭТОТ МОК В БИЗНЕС ПРОЦЕСС]
...
<br><br>
    ...
    <br><br>
    4. Не ограничивает в объемах и типах тестовых данных, открывает новый горизонт по колличеству запусков тестов.
    <br><br>
    Готовьте свою среду для отладки, ей придется нелегко [тут есть коммент, что непонятно, что это значит, мб развернуть как-то или вообще убрать, как-будто это дублирование того, что описывал в проблематике в внешних эмуляторов, да и воообще формулировка кринжовая]
    <br><br>
    5. Использование только необходимой при выполнении теста логики. [Тоже дублирование проблематики, возможно, стоит убрать и оставить только самые сильные тезисы]
    <br><br>
    6. Что может быть лучше одного стабильного теста? Два стабильных теста и мок! И тут эта проблеме решена
    <br><br>
    [это аналогично с пунктами 5-6 дублирование, но мне нравится такое окончание, я будто бы делаю много перечислений и в последнем пункте снижаю градус мемом и говорю про решенную тему из проблематики, чтобы читатель связал это со всеми болями внешних эмуляторов и понял, что все они решаются и список хоть и завершен, но формально — больше]
    <br><br>
...
[//]: # ([Табличка хз, мб стоит убрать и использовать в финальной статье цикла])
[//]: # ()
...
[//]: # (|   Не требует дополнительных ресурсов   |   3/3 ✅   |   1/3 ❌   |   2/3 ❌   |)

[//]: # (|   Простота   |   3/3 ✅   |   1/3 ❌   |   2/3 ❌   |)

[//]: # (|   Встраивание в бизнес-сервисы   |   1/3 ❌   |   3/3 ✅   |   3/3 ✅   |)

[//]: # (|   Лучшая гибкость   |   1/3 ❌   |   2/3 ❌   |   3/3 ✅   |)
...
А пока хороший доклад на конференции Heisenbug, от моего предводителя, великолепного Алексея Тишкина:
<br><br>
[эх тут бы ссылку на докладик, но вряд ли она будет]